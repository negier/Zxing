...
package com.xuebinduan.zxing.result;

import android.telephony.PhoneNumberUtils;
import com.google.zxing.Result;
import com.xuebinduan.zxing.Contents;
import com.xuebinduan.zxing.Intents;
import com.xuebinduan.zxing.LocaleManager;
import com.xuebinduan.zxing.PreferencesActivity;
import com.xuebinduan.zxing.R;
import com.xuebinduan.zxing.book.SearchBookContentsActivity;
...
public abstract class ResultHandler {
  ...
  final void addContact(String[] names,
    ...
    if (phoneNumbers != null) {
      int phoneCount = Math.min(phoneNumbers.length, com.xuebinduan.zxing.Contents.PHONE_KEYS.length);
      for (int x = 0; x < phoneCount; x++) {
        putExtra(intent, com.xuebinduan.zxing.Contents.PHONE_KEYS[x], phoneNumbers[x]);
        if (phoneTypes != null && x < phoneTypes.length) {
          int type = toPhoneContractType(phoneTypes[x]);
          if (type >= 0) {
            intent.putExtra(com.xuebinduan.zxing.Contents.PHONE_TYPE_KEYS[x], type);
          }
        }
      }
    }
    ...
    if (emails != null) {
      int emailCount = Math.min(emails.length, com.xuebinduan.zxing.Contents.EMAIL_KEYS.length);
      for (int x = 0; x < emailCount; x++) {
        putExtra(intent, com.xuebinduan.zxing.Contents.EMAIL_KEYS[x], emails[x]);
        if (emailTypes != null && x < emailTypes.length) {
          int type = toEmailContractType(emailTypes[x]);
          if (type >= 0) {
            intent.putExtra(com.xuebinduan.zxing.Contents.EMAIL_TYPE_KEYS[x], type);
          }
        }
      }
    }
  ...
  final void getDirections(double latitude, double longitude) {
    launchIntent(new Intent(Intent.ACTION_VIEW, Uri.parse("http://maps.google." +
        com.xuebinduan.zxing.LocaleManager.getCountryTLD(activity) + "/maps?f=d&daddr=" + latitude + ',' + longitude)));
  }

  // Uses the mobile-specific version of Product Search, which is formatted for small screens.
  final void openProductSearch(String upc) {
    Uri uri = Uri.parse("http://www.google." + com.xuebinduan.zxing.LocaleManager.getProductSearchCountryTLD(activity) +
        "/m/products?q=" + upc + "&source=zxing");
    launchIntent(new Intent(Intent.ACTION_VIEW, uri));
  }

  final void openBookSearch(String isbn) {
    Uri uri = Uri.parse("http://books.google." + com.xuebinduan.zxing.LocaleManager.getBookSearchCountryTLD(activity) +
        "/books?vid=isbn" + isbn);
    launchIntent(new Intent(Intent.ACTION_VIEW, uri));
  }
  ...
  private String parseCustomSearchURL() {
    ...
    String customProductSearch = prefs.getString(com.xuebinduan.zxing.PreferencesActivity.KEY_CUSTOM_PRODUCT_SEARCH,
        null);
